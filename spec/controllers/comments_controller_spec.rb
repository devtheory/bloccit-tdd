...
RSpec.describe CommentsController, type: :controller do
  ...
  let(:other_user) { User.create!(name: "User Two", email:"user2@bloc.io", password: "helloworld", role: :member) }
  let(:my_topic) { Topic.create!(name:  "A random topic title to meet validation", description: RandomData.random_paragraph) }
  ...
  context "member doing CRUD on a comment they own" do
    ...
    describe "POST create" do
      it "increates the number of comments by 1" do
        expect{ post :create, post_id: my_post.id, comment: {body: RandomData.random_paragraph}}.to change(Comment, :count).by(1)
      end
      it "redirects to the post show view" do
        post :create, post_id: my_post.id, comment: {body: RandomData.random_paragraph}
  ...
  context "admin doing CRUD on a comment they don't own" do
    before do
      other_user.update_attributes!(password: "password", role: "admin")
      create_session(other_user)
    end
