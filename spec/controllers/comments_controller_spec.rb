...
RSpec.describe CommentsController, type: :controller do
  ...
  context "member doing CRUD on a comment they own" do
    ...
    describe "POST create" do
      it "increates the number of comments by 1" do
        expect{ post :create, format: :js, format: :js, post_id: my_post.id, comment: {body: RandomData.random_paragraph}}.to change(Comment, :count).by(1)
      end
      it "redirects to the post show view" do
        post :create, format: :js, post_id: my_post.id, comment: {body: RandomData.random_paragraph}
        expect(response).to have_http_status(:success)
  ...
  context "admin doing CRUD on a comment they don't own" do
    before do
      other_user.update_attributes!( role: "admin")
      create_session(other_user)
    end
